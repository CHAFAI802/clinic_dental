...
def draw_prescription(c, prescription):
    """
    Dessine l'ordonnance sur le canvas ReportLab c, style ordonnance médicale.
    """
    ...
    # Texte du cabinet dans la bande
    ...
    c.drawString(4 * cm, height - 2.1 * cm,
                 f"Dr {medecin.first_name} {medecin.last_name} - {medecin.email} - Tél : 07 79 29 87")
    c.setFillColor(colors.black)
    ...
    # Bloc contenu ordonnance
    if prescription.text:
        c.setFont("Helvetica", 10)
        c.drawString(50, 480, "Commentaires :")
    ...
    # ---- Médicaments ----
    y = 420  # position verticale de départ
    c.setFont("Helvetica-Bold", 11)
    c.drawString(50, y, "Médicaments prescrits :")
    y -= 20
    c.setFont("Helvetica", 10)
    ...
    for item in prescription.items.all():
        ...
        y -= 15
        # si tu arrives en bas de page, gère un saut de page :
        if y < 50:
            c.showPage()
            y = 750
            c.setFont("Helvetica", 10)
    ...
    c.setFont("Helvetica", 9)
    c.drawString(1 * cm, 2.4 * cm, f"Fait à : _____________")
